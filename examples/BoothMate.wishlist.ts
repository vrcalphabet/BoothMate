import { BoothMate, Wishlist, WishlistBasic, WishlistSummary } from 'boothmate'
...
;(async () => {
  ...
  // 特定のウィッシュリストの商品一覧を取得
  const wishlistItems: Wishlist | undefined =
    await client.wishlist.getItems('pQ9TlbPV')

  // デフォルト（すべて）のウィッシュリストの商品一覧を取得
  const defaultItems: WishlistBasic = await client.wishlist.getDefaultItems()

  // 未分類のウィッシュリストの商品一覧を取得
  const uncategorizedItems: WishlistBasic =
    await client.wishlist.getUncategorizedItems()
