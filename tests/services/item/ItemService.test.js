...
describe('BoothMate.item', () => {
    describe('BoothMate.item.search', () => {
        ...
        it('検索フィルターのみで検索結果が返るか？', async () => {
            const result = await client.item.search({ category: types_1.Category.THREE_D_MODEL });
            expect(result).toBeDefined();
            expect(isObject(result)).toBe(true);
            expect(Array.isArray(result.items)).toBe(true);
        });
        it('検索クエリとフィルターを指定して検索結果が返るか？', async () => {
            const result = await client.item.search('vr', { category: types_1.Category.THREE_D_MODEL });
            expect(result).toBeDefined();
            expect(isObject(result)).toBe(true);
            expect(Array.isArray(result.items)).toBe(true);
        });
