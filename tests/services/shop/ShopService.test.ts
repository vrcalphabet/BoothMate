...
describe('BoothMate.shop', () => {
  ...
  describe('BoothMate.shop.getShopItemList', () => {
    it('有効なショップのサブドメインとアイテムリストIDでアイテムリストが返るか？', async () => {
      const result = await client.shop.getItemList(
        global.SHOP_SUBDOMAIN,
        global.ITEM_LIST_ID,
      );
      expect(result).toBeDefined();
      expect(Array.isArray(result?.items)).toBe(true);
    ...
    it('ページ省略時とpage=1指定時の結果が同値か？', async () => {
      const implicit = await client.shop.getItemList(
        global.SHOP_SUBDOMAIN,
        global.ITEM_LIST_ID,
      );
      const explicit = await client.shop.getItemList(
        global.SHOP_SUBDOMAIN,
        global.ITEM_LIST_ID,
        1,
      );
    ...
    it('存在しないショップのサブドメインでundefinedが返るか？', async () => {
      const result = await client.shop.getItemList(
        global.NON_EXISTENT_SHOP_SUBDOMAIN,
        global.ITEM_LIST_ID,
      );
      expect(result).toBeUndefined();
    });
