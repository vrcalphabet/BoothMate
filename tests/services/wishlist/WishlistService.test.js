...
describe('BoothMate.wishlist', () => {
    ...
    describe('BoothMate.wishlist.removeItem', () => {
        it('ウィッシュリストに含まれる商品IDを削除してエラーが発生しないか？', async () => {
            const result = client.wishlist.removeItem(global.ITEM_ID_INCLUDED, global.WISHLIST_ID);
            await expect(result).resolves.not.toThrow();
        });
        it('ウィッシュリストに含まれていない商品IDを削除してエラーが発生しないか？', async () => {
            const result = client.wishlist.removeItem(global.ITEM_ID_NOT_INCLUDED, global.WISHLIST_ID);
            await expect(result).resolves.not.toThrow();
        });
    ...
    describe('BoothMate.wishlist.isItemInWishlist', () => {
        it('ウィッシュリストに含まれている商品IDでtrueが返るか？', async () => {
            const result = await client.wishlist.hasItem(global.ITEM_ID_INCLUDED, global.WISHLIST_ID);
            expect(result).toBe(true);
        });
        it('ウィッシュリストに含まれていない商品IDでfalseが返るか？', async () => {
            const result = await client.wishlist.hasItem(global.ITEM_ID_NOT_INCLUDED, global.WISHLIST_ID);
...
//# sourceMappingURL=WishlistService.test.js.map